...
from rag.utils.error_handler import AIBaseException
...
async def ai_stream(request: StreamRequest):
    ...
    async def token_generator() -> AsyncGenerator[str, None]:
        try:
            ...
            from rag.planner.intent_detector import QueryPlanner
            import os
...
class ChatRequest(BaseModel):
    query: Optional[str] = Field(None, description="User's follow-up question.")
...
async def ai_chat(request: ChatRequest):
    ...
    if not user_query:
        raise HTTPException(status_code=400, detail="Query or message string is required.")

    try:
        from rag.utils.memory import get_session_memory
        ...
        # Store this turn
        memory.add_turn(user=user_query, assistant=result["answer"])
...
async def clear_chat(session_id: str):
    ...
    from rag.utils.memory import clear_session
